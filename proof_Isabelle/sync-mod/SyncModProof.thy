...
next
    ...
    hence "k_mod.SyncMod_sendMsg k q p sq = Val (x sq)"
        ...
        (simp add: `\<And>k. k_mod.SyncMod_HOMachine k \<equiv> \<lparr>CinitState = \<lambda>p st crd. k_mod.SyncMod_initState k p st,
        ...
        HOcommSchedule = k_mod.SyncMod_commSchedule\<rparr>`)
    hence "rho r q \<noteq> Asleep"
...
proof -
    ...
    next
        case non_forc:False
        thus "x ss < k"
        proof (cases "\<forall>q v. ?msgs q \<noteq> Content (Val v)")
            case True
            thus "x ss < k" using `k > 2` nxt k_mod.SyncMod_nextState_def by auto
        next
            case False
            hence "\<exists>q v. ?msgs q = Content (Val v) \<and> Suc v mod k = x ss" 
...
lemma sending_rec : assumes "p \<in> HO (Suc r) q"
and run:"HORun (HOMachine_to_Algorithm (k_mod.SyncMod_HOMachine k)) rho HO" (is "HORun ?A _ _")
...
proof -
    have "sendMsg ?A p q s = Val (x s)" using HOMachine_to_Algorithm_def k_mod.SyncMod_HOMachine_def `x s < k`
        by (simp add: `\<And>k. k_mod.SyncMod_HOMachine k \<equiv>
        \<lparr>CinitState = \<lambda>p st crd. k_mod.SyncMod_initState k p st, sendMsg = k_mod.SyncMod_sendMsg k,
        CnextState = \<lambda>p st msgs crd. k_mod.SyncMod_nextState k p st msgs, HOcommPerRd = k_mod.SyncMod_commPerRd,
        HOcommGlobal = k_mod.SyncMod_commGlobal, HOcommSchedule = k_mod.SyncMod_commSchedule \<rparr>` k_mod.SyncMod_sendMsg_def)
    thus ?thesis 
...
proof -
    ...
    proof (rule+)
        ...
        hence non_forc:"\<not> (k_mod.ready_force k ?msgs ss)"
        ...
        hence "Suc (x s) mod k \<ge> x st" using `?msgs xi = Content (Val (x s))` 
...
next
    ...
    have no_discord:"\<forall>p q vv. ?msgs p q = Content (Val vv) \<longrightarrow> vv = (v+i) mod k"
    proof (rule+)
        fix p q vv
    ...
    hence "\<forall>p. ?msgs p xi = Content (Val ((v+i) mod k))" (is "\<forall>p. ?msgs p xi = _")
        using sending_rec[of xi HO "r+Suc i" _ k rho sxx] assms sxx by (simp add:k_mod.xi_nek_def )

    have "\<forall>p ss st. rho (r+Suc i) p = Active ss \<longrightarrow> rho (Suc (r+Suc i)) p = Active st \<longrightarrow> x st = (v + Suc i) mod k"
    proof (rule+)
        fix p ss st
        ...
        proof (cases "k_mod.ready_force k (?msgs p) ss")
            case True
            hence "\<exists>p1 p2 v1 v2. (?msgs p) p1 = Content (Val v1) \<and> (?msgs p) p2 = Content (Val v2) \<and> v1 \<noteq> v2"
                using k_mod.ready_force_def by blast
            then obtain p1 p2 v1 v2 where "(?msgs p) p1 = Content (Val v1)" and
                                          "(?msgs p) p2 = Content (Val v2)" and "v1 \<noteq> v2" by auto
            ...
            thus "x st = (v + Suc i) mod k" using `v1 \<noteq> v2` by auto
        next
            case False
            moreover from `\<forall>p. ?msgs p xi = Content (Val ((v+i) mod k))` have "\<not> (\<forall>q v. (?msgs p) q \<noteq> Content (Val v))" by auto
            ultimately have "\<exists>q v. ?msgs p q = Content (Val v) \<and> Suc v mod k = x st"
...
proof -
    have "\<exists>n. Schedule rho n = UNIV" using commS by (simp add:k_mod.SyncMod_HOMachine_def k_mod.SyncMod_commSchedule_def )
    then obtain n :: nat where "\<forall>p. rho n p \<noteq> Asleep"
    ...
    have "\<forall>p. \<exists>rf sf. rho rf p = Active sf \<and> fire sf"
    proof (rule allI impI)+
        ...
        then obtain ss where "rho (Suc ?rd) p = Active ss" 
            using run HORun_def nonAsleepAgain[of rho n p _ _ _ "Suc ?rd - n"] using `\<forall>p. rho n p \<noteq> Asleep` by fastforce
        ...
        have "\<forall>q. ?msgs q = Void \<or> ?msgs q = Content (Val (k-1))"
        proof
            fix q
            obtain sq where "rho ?rd q = Active sq"
                using run HORun_def nonAsleepAgain[of rho n q _ _ _ "?rd - n"] using `\<forall>p. rho n p \<noteq> Asleep` `?rd \<ge> n` by fastforce
            moreover obtain sqq where "rho (Suc ?rd) q = Active sqq"
                using run HORun_def nonAsleepAgain[of rho n q _ _ _ "Suc ?rd - n"] using `\<forall>p. rho n p \<noteq> Asleep` `?rd \<ge> n` by fastforce
            ...
            hence "sendMsg ?A q p sqq = Val (k-1)"
                ...
                by (simp add: `\<And>k. k_mod.SyncMod_HOMachine k \<equiv> \<lparr> CinitState = \<lambda>p st crd. k_mod.SyncMod_initState k p st,
                sendMsg = k_mod.SyncMod_sendMsg k, CnextState = \<lambda>p st msgs crd. k_mod.SyncMod_nextState k p st msgs,
                HOcommPerRd = k_mod.SyncMod_commPerRd, HOcommGlobal = k_mod.SyncMod_commGlobal, HOcommSchedule = k_mod.SyncMod_commSchedule \<rparr>`)
            hence "?msgs q = (if q \<in> HO (Suc (Suc ?rd)) p then Content (Val (k-1)) else Void)"
        ...
        hence "rho (Suc (Suc ?rd)) p = Active st \<and> fire st" using `rho (Suc (Suc ?rd)) p = Active st` by auto
        thus "? rf sf. rho rf p = Active sf \<and> fire sf" by auto
    qed
    thus "k_mod.liveness rho" using k_mod.liveness_def by fastforce
qed

definition round_force where
"round_force rho p \<equiv> Eps (\<lambda>r. \<exists>sa saa. rho r p = Active sa \<and> (\<not> forc sa) \<and> rho (Suc r) p = Active saa \<and> forc saa)"
...
and "k > 2"
shows "\<not> k_mod.ready_force k (HOrcvdMsgs ?A p (HO (Suc n) p) (rho n)) sr" (is "\<not> k_mod.ready_force k ?msgs sr")
proof (cases "\<exists>r sa saa. rho r p = Active sa \<and> (\<not> forc sa) \<and> rho (Suc r) p = Active saa \<and> forc saa")
    case True
    then obtain r where "\<exists>sa saa. rho r p = Active sa \<and> (\<not> forc sa) \<and> rho (Suc r) p = Active saa \<and> forc saa" by auto
    then obtain sa saa where rd_forc:"rho (round_force rho p) p = Active sa \<and> (\<not> forc sa) \<and> rho (Suc (round_force rho p)) p = Active saa \<and> forc saa"
        using round_force_def[of rho p]  someI[of "\<lambda>r. \<exists>sa saa. rho r p = Active sa \<and> (\<not> forc sa) \<and> rho (Suc r) p = Active saa \<and> forc saa" r] by auto 
    have "\<forall>i ss. rho (Suc i + round_force rho p) p = Active ss \<longrightarrow> forc ss"
    proof (rule allI)+
        fix i ss
        show "rho (Suc i + round_force rho p) p = Active ss \<longrightarrow> forc ss"
        proof (induction i arbitrary:ss)
            case 0
            thus "rho (Suc 0 + round_force rho p) p = Active ss \<longrightarrow> forc ss" using rd_forc by auto
        next
            case (Suc ii)
            ...
            show "rho (Suc (Suc ii) + round_force rho p) p = Active ss \<longrightarrow> forc ss" 
            proof
        ...
        qed
    qed
    hence "\<forall>s. rho n p = Active sr \<longrightarrow> forc sr"
        using assms(1) allE[of "\<lambda>i. \<forall>ss. rho (Suc i + round_force rho p) p = Active ss \<longrightarrow> forc ss" "n - Suc (round_force rho p)"] by auto
    thus ?thesis using k_mod.ready_force_def[of k ?msgs sr] by (metis assms(3))
next
    case False
    hence "\<not> forc s \<or> forc sr" using assms(3) assms(4) by auto
    thus ?thesis
    proof
        assume "\<not> forc s"
...
lemma neverForce : assumes "\<not> (\<exists>sa saa. rho (round_force rho xi) xi = Active sa \<and> (\<not> forc sa) \<and> rho (Suc (round_force rho xi)) xi = Active saa \<and> forc saa)"
and run:"HORun (HOMachine_to_Algorithm (k_mod.SyncMod_HOMachine k)) rho HO" (is "HORun ?A _ _")
shows "\<forall>i. rho i xi = Active st \<longrightarrow> \<not> forc st"
proof
    fix i
    show "rho i xi = Active st \<longrightarrow> \<not> forc st"
    proof (induction i arbitrary:st)
        case 0
        show "rho 0 xi = Active st \<longrightarrow> \<not> forc st" using assms starting[of 0 rho xi ] by auto
    next
        case (Suc ii)
        show "rho (Suc ii) xi = Active st \<longrightarrow> \<not> forc st"
        proof (cases "rho ii xi")
            case Asleep
            thus "rho (Suc ii) xi = Active st \<longrightarrow> \<not> forc st" using run starting[of "Suc ii" rho xi ] by auto
        next
            case (Active sxi)
            moreover have "\<forall>sa saa l. rho l xi = Active sa \<longrightarrow> (\<not> forc sa) \<longrightarrow> rho (Suc l) xi = Active saa \<longrightarrow> (\<not> forc saa)"
                using assms(1)
                someI_ex[of "\<lambda>l. \<exists>sa saa. rho l xi = Active sa \<and> (\<not> forc sa) \<and> rho (Suc l) xi = Active saa \<and> forc saa"]
                round_force_def by (smt Eps_cong)
            ultimately show "rho (Suc ii) xi = Active st \<longrightarrow> \<not> forc st" using Suc.IH[of sxi] by auto
        qed
    qed
qed
...
proof -
    ...
    have "\<not> k_mod.ready_force k (?msgs p) sp" using nonForceAgain[of rho p "Suc r" k HO sp spp] `k > 2` assms(8) assms(9) assms(7) run less_SucI by blast
...
proof -
    ...
    moreover have "rho (r+(k - 1 - v mod k)) xi \<noteq> Asleep" using nonAsleepAgain[of rho r xi _ _ _ "k - 1 - v mod k"] assms(3) run HORun_def
    ...
    moreover from this have "rho (Suc (r+(k - 1 - v mod k))) xi \<noteq> Asleep" using nonAsleepAgain[of rho "r+(k - 1 - v mod k)" xi _ _ _ 1] run HORun_def by fastforce
...
shows "x ss = 0"
proof -
    have nforc:"\<not> k_mod.ready_force k ?msgs s" using nonForceAgain[of rho xi r k HO s ss] assms by auto
    have nxt:"k_mod.SyncMod_nextState k xi s ?msgs ss" using transition[of rho r xi s ss k HO] assms by auto
    hence "\<forall>q v. ?msgs q = Content (Val v) \<longrightarrow> Suc v mod k \<ge> x ss" using k_mod.SyncMod_nextState_def nforc by auto
...
and "k > 2"
and "n > 4"
and rd:"\<forall>p. round_force rho p < n - 4"
...
and non_forc:"\<not> (\<exists>sa saa. rho (round_force rho xi) xi = Active sa \<and> (\<not> forc sa) \<and> rho (Suc (round_force rho xi)) xi = Active saa \<and> forc saa)"
and commS:"HOcommSchedule (k_mod.SyncMod_HOMachine k) (Schedule rho)"
and "Suc (n-2) = n-1"
and "Suc (n-3) = n-2"
shows "x sxii = Suc (x sxiii) mod k"
proof -
    let ?msgx = "\<lambda>p. HOrcvdMsgs ?A xi (HO (Suc (n-1)) xi) (rho (n-1)) p"
    from rd have rd3:"\<forall>p. round_force rho p < n - 3"
    ...
    hence rd2:"\<forall>p. round_force rho p < n - 2"
        by (metis One_nat_def Suc_diff_Suc Suc_lessD add_Suc assms(4) numeral_3_eq_3 numeral_Bit0 one_add_one plus_1_eq_Suc trans_less_add2)
    hence rd1:"\<forall>p. round_force rho p < n - 1"
        by (metis One_nat_def Suc_diff_Suc Suc_lessD add_Suc assms(4) numeral_Bit0 one_add_one plus_1_eq_Suc trans_less_add2)
    hence rd0:"\<forall>p. round_force rho p < n"
    ...
    hence "\<not> k_mod.ready_force k ?msgs sxiii" using run nonForceAgain[of rho xi n k HO sxiii sxii] `k > 2` sxiii sxii rd0 by auto
    hence "forc sxiii \<or>
        (\<exists>p. ?msgs p = Content (Val (k-1))) \<or>
        (\<forall>p q v1 v2. ?msgs p = Content (Val v1) \<longrightarrow> ?msgs q = Content (Val v2) \<longrightarrow> v1 = v2)"
    ...
    next
        assume "(\<exists>p. ?msgs p = Content (Val (k-1))) \<or>
            (\<forall>p q v1 v2. ?msgs p = Content (Val v1) \<longrightarrow> ?msgs q = Content (Val v2) \<longrightarrow> v1 = v2)"
        thus ?thesis 
        proof
            assume "\<exists>p. ?msgs p = Content (Val (k-1))"
            ...
            have "\<forall>q. ?msgx q \<noteq> Content (Val (k-1))" 
            proof (rule+)
                fix q
                ...
                hence "x sqq \<le> Suc (x sxs) mod k"
                ...
                let ?msg1 = "\<lambda>p. HOrcvdMsgs ?A xi (HO (Suc (n-2)) xi) (rho (n-2)) p"
                have nxt:"k_mod.SyncMod_nextState k xi sxs ?msg1 ss"
                    using transition[of rho "n-2" xi sxs ss k] assms by auto
                have nfor:"\<not> k_mod.ready_force k ?msg1 sxs"
                    using rd2 nonForceAgain[of rho xi "n-2" k HO sxs ss] assms by auto
                hence "\<forall>qq. ?msg1 qq = Content (Val (k-1)) \<longrightarrow> x ss \<le> Suc (k-1) mod k" using nxt by (simp add:k_mod.SyncMod_nextState_def)
                hence "\<forall>qq. ?msg1 qq \<noteq> Content (Val (k-1))" using `x ss = k-2` `k > 2` by auto
                ...
                have "\<exists>ssqq sssqq. rho (n-3) qq = Active ssqq \<and> rho (n-2) qq = Active sssqq \<and> x sssqq = v"
                ...
                hence "\<exists>p1 p2 v1 v2. ?msg1 p1 = Content (Val v1) \<and> ?msg1 p2 = Content (Val v2) \<and> v1 \<noteq> v2" using `?msg1 xi = Content (Val (k-2))` `k > 2` by fastforce
                hence "forc sxs" using nfor `\<forall>qq. ?msg1 qq \<noteq> Content (Val (k-1))` k_mod.ready_force_def[of k ?msg1 sxs] `k > 2` by auto
                thus "False" using neverForce[of rho xi k HO sxs] run non_forc sxs by auto
            qed
            moreover have "\<not> k_mod.ready_force k ?msgx ss" using run nonForceAgain[of rho xi "n-1" k HO ss sxiii] `k > 2` ss sxiii rd1 by (simp add: nat_diff_split)
            ultimately have "forc ss \<or> (\<forall>p q v1 v2. ?msgx p = Content (Val v1) \<longrightarrow> ?msgx q = Content (Val v2) \<longrightarrow> v1 = v2)"
            ...
            next
                assume "\<forall>p q v1 v2. ?msgx p = Content (Val v1) \<longrightarrow> ?msgx q = Content (Val v2) \<longrightarrow> v1 = v2"
                ...
                ultimately have "\<forall>p v. ?msgx p = Content (Val v) \<longrightarrow> v = k - 2" by auto
                have "k_mod.SyncMod_nextState k xi ss ?msgx sxiii"
                    using transition[of rho "n-1" xi ss sxiii k] assms by auto
                hence "\<exists>q v. ?msgx q = Content (Val v) \<and> Suc v mod k = x sxiii"
                  using `?msgx xi = Content (Val (k-2))` `~ k_mod.ready_force k ?msgx ss`
                  unfolding k_mod.SyncMod_nextState_def by fastforce
                hence "\<exists>v. v = k - 2 \<and> Suc v mod k = x sxiii" using `\<forall>p v. ?msgx p = Content (Val v) \<longrightarrow> v = k - 2` by auto
                hence "x sxiii = k - 1" using `k > 2` by fastforce
                thus ?thesis using `x sxii = 0` `k > 2` by auto
            qed
        next
            assume "\<forall>p q v1 v2. ?msgs p = Content (Val v1) \<longrightarrow> ?msgs q = Content (Val v2) \<longrightarrow> v1 = v2"
            ...
            hence "?msgs xi = Content (Val (x sxiii))"
                ...
                by (simp add: k_mod.xi_nek_def run less_natE)
            ultimately have "\<forall>p v. ?msgs p = Content (Val v) \<longrightarrow> v = x sxiii" by auto
            have "k_mod.SyncMod_nextState k xi sxiii ?msgs sxii"
                using transition[of rho n xi sxiii sxii k] assms by auto
            moreover have "\<not> k_mod.ready_force k ?msgs sxiii"
                using run nonForceAgain[of rho xi n k HO sxiii sxii] `k > 2` sxii sxiii rd0 by simp 
            ultimately have "\<exists>q v. ?msgs q = Content (Val v) \<and> Suc v mod k = x sxii"
              using `?msgs xi = Content (Val (x sxiii))`
              unfolding k_mod.SyncMod_nextState_def by fastforce
            hence "\<exists>v. v = x sxiii \<and> Suc v mod k = x sxii" using `\<forall>p v. ?msgs p = Content (Val v) \<longrightarrow> v = x sxiii` by auto
            thus ?thesis by auto
        qed
    qed
qed
...
proof -
    have monov:"\<forall>p r s ss. rho r p = Active s \<longrightarrow> rho (Suc r) p = Active ss \<longrightarrow> fire ss \<longrightarrow> \<not> fire s \<longrightarrow> monovalent rho 0 r \<and> rho r xi \<noteq> Asleep"
    proof (rule+)
        fix p r s ss
        assume s:"rho r p = Active s" and ss:"rho (Suc r) p = Active ss" and "fire ss" and "\<not> fire s" 
        show "monovalent rho 0 r" and "rho r xi \<noteq> Asleep"
        proof -
            let ?msgs = "\<lambda>q. HOrcvdMsgs ?A p (HO (Suc r) p) (rho r) q"
            ...
            hence "rho r xi \<noteq> Asleep" using HOrcvdMsgs_def[of ?A p "HO (Suc r) p" "rho r"] assms by auto
            have "r \<noteq> 0"
            proof
                assume "r = 0"
                then obtain sz where sz:"rho 0 xi = Active sz" using `rho r xi \<noteq> Asleep` by (cases "rho 0 xi") auto
            ...
            thus "monovalent rho 0 r" and "rho r xi \<noteq> Asleep" using A2[of HO xi k rho r sxx] assms by auto
        qed
    qed
    show ?thesis
    proof (cases "\<exists>p r s ss. rho r p = Active s \<and> rho (Suc r) p = Active ss \<and> fire ss \<and> \<not> fire s")
        case False
        hence "\<forall>p rf ss sf. rho rf p = Active ss \<longrightarrow>
                    (\<not> fire ss) \<longrightarrow> rho (Suc rf) p = Active sf \<longrightarrow> fire sf \<longrightarrow> rf mod k = 0" by blast
        hence "\<exists>c. \<forall>p rf ss sf. rho rf p = Active ss \<longrightarrow>
                    (\<not> fire ss) \<longrightarrow> rho (Suc rf) p = Active sf \<longrightarrow> fire sf \<longrightarrow> rf mod k = c" by auto
        thus ?thesis by (simp add:k_mod.safety_def)
    next
        case True
        then obtain p r s ss where s:"rho r p = Active s" and ss:"rho (Suc r) p = Active ss" and "fire ss" and "\<not> fire s" by auto
        hence "monovalent rho 0 r" and "rho r xi \<noteq> Asleep" using monov by auto
        then obtain sx where "rho r xi = Active sx" by (cases "rho r xi") auto
        have "\<forall>p rf ss sf. rho rf p = Active ss \<longrightarrow>
                    (\<not> fire ss) \<longrightarrow> rho (Suc rf) p = Active sf \<longrightarrow> fire sf \<longrightarrow> rf mod k = r mod k"
        proof (rule+)
            fix q rq sq sqq
            assume sq:"rho rq q = Active sq" and sqq:"rho (Suc rq) q = Active sqq" and "\<not> fire sq" and "fire sqq"
            hence "monovalent rho 0 rq" and "rho rq xi \<noteq> Asleep" using monov by auto
...
proof -
    ...
    show ?thesis
    proof (cases "\<exists>sa saa. rho ?txi xi = Active sa \<and> (\<not> forc sa) \<and> rho (Suc ?txi) xi = Active saa \<and> forc saa")
        case True
        then obtain sx sxx where "rho ?txi xi = Active sx" and "\<not> forc sx" and "rho (Suc ?txi) xi = Active sxx" and "forc sxx" by auto
    ...
    next
        case False
        have "\<exists>n. Schedule rho n = UNIV" using commS by (simp add:k_mod.SyncMod_HOMachine_def k_mod.SyncMod_commSchedule_def )
        then obtain n :: nat where "\<forall>p. rho n p \<noteq> Asleep" unfolding Schedule_def by auto
        let ?n = "Suc (Suc (Suc (Suc (Suc n)))) + Sum ( (round_force rho) ` UNIV)"
        obtain sxi where "rho ?n xi = Active sxi" 
            using nonAsleepAgain[of rho n xi _ _ _ "Suc (Suc (Suc (Suc (Suc (Sum ( (round_force rho) ` UNIV))))))"] run HORun_def `\<forall>p. rho n p \<noteq> Asleep`
            by (smt add.commute add_Suc_shift)
        show ?thesis
        proof (cases "\<exists>v :: nat. v < k \<and> monovalent rho v ?n")
            case True
            thus ?thesis using A6[of HO xi k rho ?n sxi] assms `rho ?n xi = Active sxi` by (metis mod_if)
        next
            case False
            have escalier:"\<forall>i sxii. rho (?n + i) xi = Active sxii \<longrightarrow> x sxii = (x sxi + i) mod k"
            proof (rule allI)+
                fix i sxii
                show "rho (?n + i) xi = Active sxii \<longrightarrow> x sxii = (x sxi + i) mod k"
                proof (induction i arbitrary:sxii)
                    case 0
                    obtain sx where "rho (?n-1) xi = Active sx"
                        using nonAsleepAgain[of rho n xi _ _ _ "Suc (Suc (Suc (Suc (Sum ( (round_force rho) ` UNIV)))))"] run HORun_def `\<forall>p. rho n p \<noteq> Asleep`
                    ...
                    thus "rho (?n + 0) xi = Active sxii \<longrightarrow> x sxii = (x sxi + 0) mod k" using `rho ?n xi = Active sxi` by auto
                next
                    case (Suc ii)
                    show "rho (?n + Suc ii) xi = Active sxii \<longrightarrow> x sxii = (x sxi + Suc ii) mod k"
                    proof 
                        assume "rho (?n + Suc ii) xi = Active sxii"
                        hence "rho (Suc ?n + ii) xi = Active sxii" by auto
                        moreover have "\<forall>p. round_force rho p < ?n +ii - 4"
                        proof
                            fix p
                            have rd_forc:"round_force rho p \<le> Sum ((round_force rho) ` UNIV)"
                                by (meson finite_UNIV finite_imageI le0 range_eqI sum_nonneg_leq_bound)
                            hence "round_force rho p \<le> Sum ((round_force rho) ` UNIV) + ((Suc (Suc (Suc (Suc n)))) + ii - 4)" using trans_le_add1 by blast
                            moreover have "Suc (Suc (Suc (Suc n))) + ii \<ge> 4" by auto
                            ...
                            ultimately have "round_force rho p \<le> Sum ((round_force rho) ` UNIV) + (Suc (Suc (Suc (Suc n)))) + ii - 4"
                                by fastforce
                            thus "round_force rho p < ?n + ii - 4" by auto
                        qed
                        moreover obtain sxx where "rho (n+Suc (Suc (Sum ( (round_force rho) ` UNIV)))+ii) xi = Active sxx"
                            using nonAsleepAgain[of rho n xi _ _ _ "Suc (Suc (Sum ((round_force rho) ` UNIV)))+ii"] run HORun_def `\<forall>p. rho n p \<noteq> Asleep`
                            by (metis add.assoc)
                        ...
                        ultimately have "x sxii = Suc (x sxiii) mod k"
                            ...
                            `~ (\<exists>sa saa. rho ?txi xi = Active sa \<and> (~ forc sa) \<and> rho (Suc ?txi) xi = Active saa \<and> forc saa)` by fastforce
            ...
            obtain sxx where "rho (?n-1) xi = Active sxx" 
                ...
                run HORun_def add_diff_assoc[of 1 "Suc (Suc (Suc (Suc (Suc (Sum (range (round_force rho)))))))" n] `\<forall>p. rho n p \<noteq> Asleep`
            ...
            hence "x s = k - 1" using `k - 1 \<ge> x sxi` by (simp add: add_diff_inverse_nat nat_diff_split)
