...
shows "\<forall>q. HOrcvdMsgs (k_mod.gen_HOMachine k) q (HO (Suc r) q) (rho r) p
    = (if p \<in> HO (Suc r) q then Content s else Void)" using s
...
proof (cases "rho r q")
  case Asleep
  hence "?msgs q = (if q \<in> HO (Suc r) p then Bot else Void)" unfolding HOrcvdMsgs_def by auto
...
proof -
  ...
  from run have "CHOinitConfig (k_mod.gen_HOMachine k) rho (\<lambda>_ _. undefined)" (is "CHOinitConfig ?A _ _")
...
proof -
    ...
    then obtain sss where "rho (Suc r) p = Active sss \<and> CnextState ?A p s ?msgs undefined sss" using s_def 
...
proof -
  ...
  have "(\<exists>s. rho (Suc r) q = Active s \<and> (Suc (x s)) mod k = x sp mod k) \<and>
    k_mod.isConc k ?msg \<and>
    (k_mod.isReady ?msgs \<longrightarrow> k_mod.isReady ?msg)"
  proof (cases "rho (Suc r) q")
    case Asleep
    hence "?msgs q = Bot" 
      unfolding HOrcvdMsgs_def using assms by auto
    hence "\<not> k_mod.isConc k ?msgs" unfolding k_mod.isConc_def by (metis message.distinct(1) message.distinct(5))
  ...
  next
    ...
    proof (cases "k_mod.ready_level1 k ?msgs ssp \<or> k_mod.ready_level2 k ?msgs ssp")
      case levlup:True
      ...
      moreover have "x sp = 0 \<or> x sp = Suc (k_mod.minMsgs ?msgs)" using levlup trans unfolding k_mod.gen_nextState_def by auto
    ...
    qed
    ultimately have "\<exists>s. rho (Suc r) q = Active s \<and> (Suc (x s)) mod k = x sp mod k"
      using `rho (Suc r) q = Active sq`
      exI[of "\<lambda>s. rho (Suc r) q = Active s \<and> Suc (x s) mod k = x sp mod k" sq] by (metis mod_Suc_eq)
    moreover from coc have "conc sq" unfolding k_mod.isConc_def
      by (metis `?msgs q = Content sq` message.distinct(3) message.inject)
    hence "rho r q \<noteq> Asleep" using starting[of "Suc r" rho q k HO sq] run Active
    ...
    from sucx xsq have "x sq mod k > 0" using `x sp mod k \<noteq> 1`
    ...
    moreover from transq `x sq mod k > 0` have "\<not> (k_mod.ready_level1 k ?msg ssq \<or> k_mod.ready_level2 k ?msg ssq)" 
    ...
    from this transq `x sq mod k > 0` assms k_mod.gen_nextState_def
      have "ready sq \<longrightarrow> k_mod.isReady ?msg" by auto 
    hence "k_mod.isReady ?msgs \<longrightarrow> k_mod.isReady ?msg"
  ...
  thus "k_mod.isConc k ?msg"
    and "k_mod.isReady ?msgs \<longrightarrow> k_mod.isReady ?msg"
    and "\<exists>s. rho (Suc r) q = Active s \<and> (Suc (x s)) mod k = x sp mod k" by auto
qed
...
and "k > 2"
and "i < k"
shows "\<exists>sxi. rho (t+i) xi = Active sxi \<and> x sxi mod k = i \<and>
  (k_mod.isReady (HOrcvdMsgs (k_mod.gen_HOMachine k) p (HO (t+k) p) (rho (t+k-1))) \<longrightarrow> ready sxi)"
proof -
  from path obtain seq where "seq 0 = xi" and "seq (k-i) = p" and
    pat:"\<forall>j < k-i. seq j \<in> HO (t+i+Suc j) (seq (Suc j))" unfolding k_mod.path_def by auto
  have "\<forall>j < k-i. let msgs = HOrcvdMsgs ?A (seq (k-i-j)) (HO (t+k-j) (seq (k-i-j))) (rho (t+k-Suc j)) in
      \<exists>s. rho (t+k-j) (seq (k-i-j)) = Active s \<and> x s mod k = (k - j) mod k \<and> k_mod.isConc k msgs \<and> (k_mod.isReady ?msgs \<longrightarrow> k_mod.isReady msgs)"
      (is "\<forall>j. ?recurr_path j")
  proof
    fix j
    let ?msg = "\<lambda>j. HOrcvdMsgs ?A (seq (k-i-j)) (HO (t+k-j) (seq (k-i-j))) (rho (t+k-Suc j))"
    show "j < k-i \<longrightarrow> (let msgs = ?msg j in \<exists>s. rho (t+k-j) (seq (k-i-j)) = Active s \<and>
      x s mod k = (k-j) mod k \<and> k_mod.isConc k msgs \<and> (k_mod.isReady ?msgs \<longrightarrow> k_mod.isReady msgs))"
    proof (induction j)
      case 0
      from assms `seq (k-i) = p` have "rho (t+k) (seq (k-i)) = Active sp \<and>
        x sp mod k = (k-0) mod k \<and> k_mod.isConc k ?msgs \<and> (k_mod.isReady ?msgs \<longrightarrow> k_mod.isReady ?msgs)" by auto
      thus "0 < k-i \<longrightarrow> (let msgs = ?msg 0 in \<exists>s. rho (t+k-0) (seq (k-i-0)) = Active s \<and>
        x s mod k = (k-0) mod k \<and> k_mod.isConc k msgs \<and> (k_mod.isReady ?msgs \<longrightarrow> k_mod.isReady msgs))"
    ...
    next
      ...
      proof
        ...
        ineg3:"t+k \<ge> Suc j" and ineg4:"Suc (t+k-Suc j) = t+k-j" and ineg5:"Suc (k - Suc(i+j)) = k-i-j" by auto
        from this pat have link:"seq (k-i-Suc j) \<in> HO (t+k-j) (seq (k-i-j))" by auto

        from Suc.IH obtain ss where ss:"rho (t+k-j) (seq (k-i-j)) = Active ss"
          and concss:"k_mod.isConc k (?msg j)" and modss:"x ss mod k = (k-j) mod k" and readyss:"k_mod.isReady ?msgs \<longrightarrow> k_mod.isReady (?msg j)"
          using `Suc j < k-i` by (meson Suc_lessD)

        have "x ss mod k \<noteq> 1"
        proof
          assume "x ss mod k = 1"
        ...
        from assms pat this ss concss modss readyss
          obtain s where "rho (t+k-Suc j) (seq (k-i-Suc j)) = Active s \<and> Suc (x s) mod k = x ss mod k"
          and "k_mod.isReady ?msgs \<longrightarrow> k_mod.isReady (?msg (Suc j))" and "k_mod.isConc k (?msg (Suc j))"
            using path_conc[of rho "t+k-Suc (Suc j)" "seq (k-i-j)" ss k HO "seq (k-i-Suc j)"]
            by (smt One_nat_def Suc_diff_Suc ineg4 `Suc j < k - i` `Suc j \<le> t + k` link ineg2
            diff_self_eq_0 ineg le_eq_less_or_eq less_diff_conv not_add_less2 plus_1_eq_Suc)
        thus "let msgs = ?msg (Suc j) in \<exists>s. rho (t+k-Suc j) (seq (k-i-Suc j)) = Active s \<and>
          x s mod k = (k-Suc j) mod k \<and> k_mod.isConc k msgs \<and> (k_mod.isReady ?msgs \<longrightarrow> k_mod.isReady msgs)"
  ...
  ultimately have seq1:"let msgs = HOrcvdMsgs ?A (seq 1) (HO (t+i+1) (seq 1)) (rho (t+i)) in \<exists>ss. 
      rho (t+i+1) (seq 1) = Active ss \<and> x ss mod k = (i+1) mod k \<and> k_mod.isConc k msgs \<and>
      (k_mod.isReady ?msgs \<longrightarrow> k_mod.isReady msgs)" using `2 < k` allE[of ?recurr_path "k-i-1" "?recurr_path (k-i-1)"] by auto
  let ?msgxi = "HOrcvdMsgs ?A (seq 1) (HO (t+i+1) (seq 1)) (rho (t+i))"
  have seq1link:"xi \<in> HO (t+i+1) (seq 1)" using pat `seq 0 = xi` `i < k` by fastforce
  from seq1 have concready:"k_mod.isConc k ?msgxi \<and> (k_mod.isReady ?msgs \<longrightarrow> k_mod.isReady ?msgxi)" by meson
  have "rho (t+i) xi \<noteq> Asleep"
  proof 
    assume "rho (t+i) xi = Asleep"
    hence "?msgxi xi = Bot" using seq1link unfolding HOrcvdMsgs_def by auto 
    hence "\<not> k_mod.isConc k ?msgxi" unfolding k_mod.isConc_def by (metis message.distinct(1) message.distinct(5))
  ...
  from sxi concready this have "k_mod.isReady ?msgs \<longrightarrow> ready sxi" unfolding k_mod.isReady_def by auto
  from concready loop have "rho (t+i) (seq 1) \<noteq> Asleep" unfolding k_mod.isConc_def HOrcvdMsgs_def by fastforce
  ...
  thus "\<exists>sxi. rho (t+i) xi = Active sxi \<and> x sxi mod k = i \<and> (k_mod.isReady ?msgs \<longrightarrow> ready sxi)" using sxi `k_mod.isReady ?msgs \<longrightarrow> ready sxi` by auto
qed

lemma A2:
assumes star:"\<forall>p n. k_mod.path HO xi p n k"
and loop:"\<forall>p r. p \<in> HO r p"
and run: "HORun (k_mod.gen_HOMachine k) rho HO" (is "HORun ?A _ _")
and "k > 2"
and "i < k"
and ss:"rho i pp = Active ss"
shows "level ss = 0 \<and> x ss \<le> i \<and> forc ss = 1" using `i < k` ss
...
next
case (Suc ii)
  show "level ss = 0 \<and> x ss \<le> Suc ii \<and> forc ss = 1"
  proof (cases "rho ii pp")
    case Asleep
    thus "level ss = 0 \<and> x ss \<le> Suc ii \<and> forc ss = 1" using run starting[of "Suc ii" rho pp k HO] Suc
  ...
  next
    ...
    have "k_mod.forceMsgs (?msgp pp) \<in> k_mod.forceMsgs ` (range ?msgp)" by blast
    ...
    ultimately have "k_mod.maxForce ?msgp \<ge> 1" unfolding k_mod.maxForce_def by simp
    moreover have a:"\<forall>p s. ?msgp p = Content s \<longrightarrow> x s \<le> ii \<and> level s = 0 \<and> forc s = 1" using Suc.IH sending_rec by (meson `Suc ii < k` lessI less_trans run)
    hence "\<forall>p. k_mod.forceMsgs (?msgp p) \<le> 1" 
    ...
    ultimately have pp_inf:"?msgp pp = Content sss \<and> forc sss = k_mod.maxForce ?msgp \<and> x sss \<le> ii" using a by auto
    let ?prop = "\<lambda>sss pp ii. ?msgp pp = Content sss \<and> forc sss = k_mod.maxForce ?msgp \<and> x sss = ii"
    from pp_inf have "?prop sss pp (x sss)" and "x sss \<le> ii" by auto
    from pp_inf have "(LEAST i. \<exists>s p. ?prop s p i) \<le> x sss" using Least_le by (smt (verit, del_insts))
    hence "k_mod.minMsgs ?msgp \<le> ii" using `x sss \<le> ii` unfolding k_mod.minMsgs_def by auto
    moreover from this `Suc ii < k` Active have non_levelup:"\<not> (k_mod.ready_level1 k ?msgp sss \<or> k_mod.ready_level2 k ?msgp sss)"
      unfolding k_mod.ready_level2_def k_mod.ready_level1_def by auto
    ultimately have "x ss \<le> Suc ii" using transp unfolding k_mod.gen_nextState_def by auto
...
lemma A2_bis:
assumes loop:"\<forall>p r. p \<in> HO r p"
and run: "HORun (k_mod.gen_HOMachine k) rho HO" (is "HORun ?A _ _")
and "k > 2"
and "rho r p = Active s"
shows "forc s \<ge> 1 \<and> level s \<le> 2" using `rho r p = Active s`
...
case (Suc rr)
  ...
  proof (cases "rho rr p")
    case Asleep
    thus "forc s \<ge> 1 \<and> level s \<le> 2" using starting[of "Suc rr" rho p k HO s] Suc assms unfolding k_mod.gen_initState_def by auto
  next
    case (Active ss)
    let ?msgp = "HOrcvdMsgs ?A p (HO (Suc rr) p) (rho rr)"
    have "?msgp p = Content ss" using Active loop sending run by presburger
    have "k_mod.forceMsgs (?msgp p) \<in> k_mod.forceMsgs ` (range ?msgp)" by blast
    moreover have "k_mod.forceMsgs (?msgp p) \<ge> 1" using Suc.IH[of p ss] Active `?msgp p = Content ss` by (simp add: k_mod.forceMsgs.simps(1))
    ultimately have "k_mod.maxForce ?msgp \<ge> 1" unfolding k_mod.maxForce_def by (meson Max_ge_iff UNIV_not_empty finite finite_imageI image_is_empty)
    moreover have transp:"k_mod.gen_nextState k p ss ?msgp s" using Active assms transition Suc by auto
    hence "forc s \<ge> k_mod.maxForce ?msgp" unfolding k_mod.gen_nextState_def by auto
    ultimately show "forc s \<ge> 1 \<and> level s \<le> 2" using transp Suc.IH[of p ss] Active unfolding k_mod.gen_nextState_def by auto
  qed
qed
...
proof (rule ccontr)
  ...
  hence val_xi:"\<exists>sxi. rho (Suc r+i-k) xi = Active sxi \<and> x sxi mod k = i"
    using A1[of HO xi xi "Suc r-k" i k rho ssp] assms `Suc r \<ge> k` `k_mod.isConc k ?msgp` `~ i \<ge> k` self_path by auto
  ...
  hence lev1q:"k_mod.ready_level1 k ?msgq sq \<or> k_mod.ready_level2 k ?msgq sq" using assms transp
  ...
  hence "\<exists>sxi. rho (Suc r+i-k) xi = Active sxi \<and> x sxi mod k = 0" using A1[of HO xi q "Suc r-k+i" 0 k rho ssq] assms `Suc r \<ge> k` `k_mod.isConc k ?msgq` by auto
  thus "False" using val_xi `~ i \<ge> k` `i > 0` by auto
qed

definition active_path where 
"active_path rho HO p q n D \<equiv> \<exists>seq. seq 0 = p \<and> seq D = q \<and>
    (\<forall>i < D. rho (n+i) (seq (Suc i)) \<noteq> Asleep \<and> seq i \<in> HO (n+Suc i) (seq (Suc i)))"
...
shows "active_path rho HO p q t r \<longrightarrow> rho (t+r) q = Active sq \<longrightarrow> forc sp \<le> forc sq"
proof (induction r arbitrary:sq q)
  case 0
  show "active_path rho HO p q t 0 \<longrightarrow> rho (t+0) q = Active sq \<longrightarrow> forc sp \<le> forc sq" using sp unfolding active_path_def by fastforce
next
  case (Suc rr)
  show "active_path rho HO p q t (Suc rr) \<longrightarrow> rho (t+Suc rr) q = Active sq \<longrightarrow> forc sp \<le> forc sq"
  proof (rule impI)+
    assume "active_path rho HO p q t (Suc rr)" and sq:"rho (t+Suc rr) q = Active sq"
    then obtain seq where seq0:"seq 0 = p" and "seq (Suc rr) = q" and seqSuc:"\<forall>j < Suc rr. rho (t+j) (seq (Suc j)) \<noteq> Asleep \<and> seq j \<in> HO (t+Suc j) (seq (Suc j))"
      unfolding active_path_def by auto
    have "rho (t+rr) (seq rr) \<noteq> Asleep"
    proof (cases "rr")
      case 0
      thus "rho (t+rr) (seq rr) \<noteq> Asleep" using seq0 sp by auto
    next
      case (Suc rrr)
      hence "rho (t+rr-1) (seq rr) \<noteq> Asleep" using `seq (Suc rr) = q` seqSuc by auto
      thus "rho (t+rr) (seq rr) \<noteq> Asleep" using run nonAsleepAgain
    ...
    have "seq rr \<in> HO (t+Suc rr) q" using seqSuc `seq (Suc rr) = q` by auto
    hence "active_path rho HO p (seq rr) t rr" using seq0 seqSuc unfolding active_path_def by auto
    hence "forc sp \<le> forc sqq" using sqq Suc.IH by simp
    ...
    hence maxf:"k_mod.maxForce ?msgs \<ge> forc sp" using `forc sp \<le> forc sqq` unfolding k_mod.maxForce_def
    ...
    thus "forc sp \<le> forc sq" using maxf unfolding k_mod.gen_nextState_def by auto
  qed
qed
...
assumes "seq 0 = p"
and "seq k = q"
and "\<forall>j < k. seq j \<in> HO (n+Suc j) (seq (Suc j))"
and "i \<le> k"
shows "k_mod.path HO (seq i) q (n+i) (k-i)"
proof  -
  have "\<forall>j < k-i. seq (i+j) \<in> HO (n+i+Suc j) (seq (i+Suc j))" using assms(3) 
      by (smt (z3) add.commute add.left_commute add_Suc_right less_diff_conv)
  thus ?thesis using assms exI[of _ "\<lambda>l. seq(i+l)"] unfolding k_mod.path_def by auto
qed
...
shows "\<exists>q sq sqq. rho (Suc r-c) q = Active sqq \<and> level sqq = f-1 \<and> (if f = 1 then c \<le> r \<longrightarrow> rho (r-c) q = Asleep else rho (r-c) q = Active sq \<and> level sq = f-2)"
...
next
  ...
  have "p \<in> HO (Suc r) p" using loop by auto
  hence "msgp p = Content sp" using sp run sending cc unfolding msgp_def by simp
  have "forc sp \<ge> 1" using A2_bis run cc(5) loop `2 < k` by blast
  hence "k_mod.forceMsgs (msgp p) > 0" using `msgp p = Content sp` by (simp add:k_mod.forceMsgs.simps(1))
  moreover have "k_mod.forceMsgs (msgp p) \<in> k_mod.forceMsgs ` range msgp" by auto
  ultimately have "k_mod.maxForce msgp \<ge> 1" using Max_ge[of "k_mod.forceMsgs ` range msgp" "k_mod.forceMsgs (msgp p)"]
    unfolding k_mod.maxForce_def by (metis One_nat_def finite finite_imageI le0 le_antisym not_le not_less_eq_eq)
  hence "f \<ge> 1" using cc unfolding msgp_def by auto
  ...
  from this obtain sq where "msgp q = Content sq" using `f \<ge> 1` by (metis One_nat_def k_mod.forceMsgs.elims not_less_eq_eq order_refl)
  hence "\<exists>vv s sq. msgp q = Content sq \<and> forc sq = k_mod.maxForce msgp \<and> x sq = vv"
  ...
  from this obtain qq sqq where "msgp qq = Content sqq" and "forc sqq = k_mod.maxForce msgp" and "x sqq = cc"
    using LeastI_ex[of "\<lambda>cc. \<exists>sqq qq. msgp qq = Content sqq \<and> forc sqq = k_mod.maxForce msgp \<and> x sqq = cc"] cc(2) unfolding k_mod.minMsgs_def msgp_def by auto
  ...
  proof (cases r)
    ...
    proof (cases cc)
      ...
      next
        ...
        hence "k_mod.ready_level1 k msgq sqqq \<or> k_mod.ready_level2 k msgq sqqq" and "forc sqq = Suc (level sqq)"
    ...
    next
      ...
      ultimately have postih:"\<exists>tr srr sr. rho (Suc r - Suc cc) tr = Active sr \<and> level sr = f - 1 \<and>
       (if f = 1 then cc \<le> r - 1 \<longrightarrow> rho (r - Suc cc) tr = Asleep else rho (r - Suc cc) tr = Active srr \<and> level srr = f - 2)"
...
next
  ...
  next
    ...
    hence "k_mod.maxForce msgp \<ge> forc spp" unfolding k_mod.maxForce_def by (metis Max_ge finite_UNIV finite_imageI image_eqI k_mod.forceMsgs.simps(1) rangeI)
    moreover have transp:"k_mod.gen_nextState k p spp msgp sp" using Active assms Suc transition unfolding msgp_def by auto
    ultimately have "forc sp \<ge> forc spp \<or> forc sp = Suc (level sp)" unfolding k_mod.gen_nextState_def by presburger
...
and lvup:"\<forall>sqq. t > 0 \<longrightarrow> rho (t-1) q = Active sqq \<longrightarrow> level sqq < level sq"
...
shows "Suc (level sq) < k_mod.maxForce msgp \<or> (Suc (level sq) = k_mod.maxForce msgp \<and> (k_mod.minMsgs msgp) \<le> i)"
  using path sp msgp
proof (induction i arbitrary:p sp msgp)
  case 0
  hence "q \<in> HO (Suc t) p" unfolding active_path_def by auto
  ...
  next
    case False
    have "k_mod.maxForce msgp \<ge> forc sq" using msgq unfolding k_mod.maxForce_def
    ...
    hence "k_mod.minMsgs msgp \<le> x sq" using msgq unfolding k_mod.minMsgs_def by (metis (mono_tags, lifting) Least_le)
    thus ?thesis
    proof (cases "t > 0 \<longrightarrow> rho (t-1) q = Asleep")
      case True
      hence "x sq = 0" using run sq starting unfolding k_mod.gen_initState_def by (metis locState.select_convs(1))
      thus ?thesis  using eq `k_mod.minMsgs msgp \<le> x sq` by auto
    next
      case False
      ...
      thus ?thesis using `k_mod.minMsgs msgp \<le> x sq` eq by auto
    qed
  qed
next
  case (Suc ii)
  from Suc(2) obtain seq where "seq 0 = q" and "seq (Suc (Suc ii)) = p" and
    seq:"\<forall>j < Suc (Suc ii). rho (t+j) (seq (Suc j)) \<noteq> Asleep \<and> seq j \<in> HO (t+Suc j) (seq (Suc j))"
    unfolding active_path_def by auto
  ...
  from seq obtain ss where ss:"rho (t+ii) (seq (Suc ii)) = Active ss" and "seq (Suc ii) \<in> HO (t+Suc (Suc ii)) p"
  ...
  have ih:"Suc (level sq) < k_mod.maxForce msgs \<or> (Suc (level sq) = k_mod.maxForce msgs \<and> k_mod.minMsgs msgs \<le> ii)"
    using Suc.IH ss `active_path rho HO q (seq (Suc ii)) t (Suc ii)` msgs_def by auto
  have "k_mod.maxForce msgs \<le> forc sss" and "x sss \<le> Suc (k_mod.minMsgs msgs)" using transs unfolding k_mod.gen_nextState_def by auto
  ...
  next
    case False
    have "Suc (level sq) \<le> k_mod.maxForce msgs" using ih by auto
    moreover have "k_mod.maxForce msgp \<ge> forc sss" using `msgp (seq (Suc ii)) = Content sss`
    ...
    ultimately have "Suc (level sq) = k_mod.maxForce msgp" and "forc sss = k_mod.maxForce msgp"
      using False `k_mod.maxForce msgs \<le> forc sss` send by auto
    moreover from this have "k_mod.minMsgs msgp \<le> x sss" using `msgp (seq (Suc ii)) = Content sss`
      unfolding k_mod.minMsgs_def by (metis (mono_tags, lifting) Least_le)
    ultimately show ?thesis using `x sss \<le> Suc (k_mod.minMsgs msgs)` `k_mod.maxForce msgs \<le> forc sss` ih by linarith
  qed
qed
...
and "forc ss \<le> k_mod.maxForce (HOrcvdMsgs (k_mod.gen_HOMachine k) p (HO (t+Suc i) p) (rho (t+i)))"
proof -
  ...
  hence lev2p:"k_mod.ready_level1 k msgp sp \<or> k_mod.ready_level2 k msgp sp" using assms transp unfolding k_mod.gen_nextState_def by simp 
  ...
  from star obtain seq where "seq 0 = xi" and "seq k = p" and
    pat:"\<forall>j < k. seq j \<in> HO (Suc t+i-k+Suc j) (seq (Suc j))" (is "\<forall>j < _. _ \<in> ?HOseq j") unfolding k_mod.path_def by meson
  have nopass:"\<forall>j < k. rho (Suc t+i-k+j) (seq (Suc j)) \<noteq> Asleep"
  proof (rule+)
    fix j
    ...
    proof (cases "Suc j = k")
      case True
      thus "False" using `rho (t+i) p = Active sp` `rho (Suc t+i-k+j) (seq (Suc j)) = Asleep` `i \<ge> k` `seq k = p` by fastforce
    next
      case False
      ...
      ultimately obtain ss where ss:"rho (Suc t+i-k+Suc j) (seq (Suc j)) = Active ss \<and> x ss mod k = Suc j"
        using assms `i \<ge> k` `k_mod.isConc k msgp` A1[of HO "seq (Suc j)" p "Suc t+i-k" "Suc j" k rho ssp] `j < k` False unfolding msgp_def by auto
      hence "ss \<noteq> k_mod.gen_initState" using `rho (Suc t+i-k+j) (seq (Suc j)) = Asleep` starting unfolding k_mod.gen_initState_def by auto
      thus "False" using `rho (Suc t+i-k+j) (seq (Suc j)) = Asleep` starting[of "t+i-k+j"] `i \<ge> k` run ss
        by (metis add_Suc_right diff_Suc_1 starting)
    qed
  qed
  hence "rho (t+i-1) (seq (k-1)) \<noteq> Asleep" using `2 < k` `k \<le> i`
  ...
  from this obtain sm where sm:"rho (t+i) (seq (k-1)) = Active sm"
    using nonAsleepAgain[of rho "t+i-1" "seq (k-1)" _ _ _ 1] assms `k \<le> i` unfolding HORun_def by fastforce
  let ?exseq = "\<lambda>l. if l \<le> i-k then xi else seq (l - (i-k))"
  have extrem:"?exseq 0 = xi \<and> ?exseq i = p" using `seq k = p` `k \<le> i` assms(4) by force 
  moreover have expat:"\<forall>l < i. rho (Suc t+l) (?exseq (Suc l)) \<noteq> Asleep \<and> ?exseq l \<in> HO (Suc (Suc t)+l) (?exseq (Suc l))"
  proof (rule allI impI)+
    fix l
    assume "l < i"
    show "rho (Suc t+l) (?exseq (Suc l)) \<noteq> Asleep \<and> ?exseq l \<in> HO (Suc (Suc t)+l) (?exseq (Suc l))" 
    proof (cases "l < i-k")
      case True
      hence "rho (Suc t+l) (?exseq (Suc l)) \<noteq> Asleep" using nonAsleepAgain[of rho "Suc t" xi] run
        unfolding HORun_def by (smt (verit) Suc_leI assms(8) proc_state.simps(3))
      thus "rho (Suc t+l) (?exseq (Suc l)) \<noteq> Asleep \<and> ?exseq l \<in> HO (Suc (Suc t)+l) (?exseq (Suc l))" using loop True by auto
    next
      case False
      hence "l-(i-k) < k" using `i \<ge> k` `2 < k` by (metis `l < i` diff_diff_cancel diff_le_self less_diff_iff not_le_imp_less)
      hence "seq (l-(i-k)) \<in> HO (Suc t+i-k+(Suc l-(i-k))) (seq (Suc l-(i-k)))" using pat False Suc_diff_le by auto
      hence "?exseq l \<in> HO (Suc t+Suc l) (?exseq (Suc l))"
        using pat `seq 0 = xi` `l-(i-k) < k` False
        by (smt (z3) Nat.add_diff_assoc `i \<ge> k`  diff_is_0_eq' group_cancel.add1 le_add_diff_inverse nat_less_le not_less_eq)

      moreover have "rho (Suc t+l) (?exseq (Suc l)) \<noteq> Asleep" using `\<forall>j < k. rho (Suc t+i-k+j) (seq (Suc j)) \<noteq> Asleep` False `l-(i-k) < k` `i \<ge> k`
        by (smt (z3) Nat.add_diff_assoc Suc_eq_plus1 Suc_le_eq add.commute add.left_commute leI le_add_diff_inverse2)
      ultimately show "rho (Suc t+l) (?exseq (Suc l)) \<noteq> Asleep \<and> ?exseq l \<in> HO (Suc (Suc t)+l) (?exseq (Suc l))" by simp
    qed
  qed
  ultimately have patt:"active_path rho HO xi p (Suc t) i" using `seq k = p`
    exI[of "\<lambda>seqq. seqq 0 = xi \<and> seqq i = p \<and> (\<forall>j < i. rho (Suc t+j) (seqq (Suc j)) \<noteq> Asleep \<and> seqq j \<in> HO (Suc t+Suc j) (seqq (Suc j)))" ?exseq]
    unfolding active_path_def by auto
  have "\<forall>j < i-1. rho (Suc t+j) (?exseq (Suc j)) \<noteq> Asleep \<and> ?exseq j \<in> HO (Suc t+Suc j) (?exseq (Suc j))" using expat by auto
  hence "active_path rho HO xi (seq (k-1)) (Suc t) (i - 1)" using `seq k = p`
    exI[of "\<lambda>seqq. seqq 0 = xi \<and> seqq (i-1) = seq (k-1) \<and>
    (\<forall>j < i-1. rho (Suc t+j) (seqq (Suc j)) \<noteq> Asleep \<and> seqq j \<in> HO (Suc t+Suc j) (seqq (Suc j)))" ?exseq] extrem expat `k \<le> i` `seq 0 = xi`
    unfolding active_path_def by auto
  hence "forc sm \<ge> forc ss" using increasing_force[of HO xi k rho "Suc t" xi ss "seq (k-1)" "i-1" sm] sm assms
    by (metis Suc_diff_1 `k \<le> i` add_Suc_shift dual_order.strict_trans1 less_nat_zero_code neq0_conv)
  have "seq (k-1) \<in> HO (Suc t+i) p" using pat `seq k = p`
  ...
  hence "k_mod.maxForce msgp \<ge> forc ss" using `forc sm \<ge> forc ss`
...
shows "level s \<le> level ss" using `rho (r+i) p = Active ss`
...
next
  ...
  hence "level ss \<ge> level sss" unfolding k_mod.ready_level2_def k_mod.ready_level1_def k_mod.gen_nextState_def by auto
  thus "level s \<le> level ss" using Suc.IH sss by auto
qed
...
shows "i mod k = 0"
proof (rule ccontr)
  assume "\<not> i mod k = 0"
  have "level sp < level ssp" using assms by auto
  let ?n = "LEAST nn. \<exists>s. rho nn xi = Active s \<and> level s > 0"
  define j where "j = (LEAST jj. \<exists>q sq ssq. rho (t+jj) q = Active sq \<and>
    rho (t+Suc jj) q = Active ssq \<and> level sq < level ssq \<and> jj mod k \<noteq> 0)" (is "j = (LEAST jj. \<exists>qj sj sjj. ?P jj qj sj sjj)")
  hence "\<exists>i q sq ssq. rho (t+ i) q = Active sq \<and> rho (t+Suc i) q = Active ssq \<and> level sq < level ssq \<and> i mod k \<noteq> 0"
  ...
  from this obtain q sq ssq where sq:"rho (t+j) q = Active sq" and ssq:"rho (t+Suc j) q = Active ssq"
    and levsq:"level sq < level ssq" and "j mod k \<noteq> 0"
    using LeastI_ex[of "\<lambda>jj. \<exists>qj sj sjj. ?P jj qj sj sjj"] unfolding j_def by blast
  ...
  hence "j-Suc c mod k \<noteq> 0"
    using `j mod k \<noteq> 0` by (metis One_nat_def Suc_1 Suc_lessD Suc_pred assms(4) diff_zero mod_0 mod_Suc)

  have "i \<ge> k" using assms A3[of HO xi k rho t s ss i p sp ssp] by (metis Suc_1 `i mod k \<noteq> 0` bits_mod_0 lessI neq0_conv)
  have "j \<ge> k" using assms sq ssq levsq A3[of HO xi k rho t s ss j q sq ssq] by (metis `j mod k \<noteq> 0` bits_mod_0 neq0_conv)



  obtain h sh shh where shh:"rho (t+j-c) h = Active shh" and "level shh = f-1" and
    sh_pre:"if f = 1 then Suc c \<le> t+j \<longrightarrow> rho (t+j-Suc c) h = Asleep else rho (t+j-Suc c) h = Active sh \<and> level sh = f-2"
  ...
  have "forc ss \<le> f" and "active_path rho HO xi q (Suc t) j" using extend_path[of HO xi k rho t j q sq ssq ss]
    assms sq ssq levsq `j \<ge> k` unfolding msgq_def f_def by auto
  hence "1 < f" using level_force[of HO k rho "Suc t" xi ss] sq assms by auto
  hence "t+j \<ge> Suc c" using sh_pre
  ...
  hence "k_mod.ready_level1 k msgh sh \<or> k_mod.ready_level2 k msgh sh" using `level shh = f-1` sh_pre `1 < f` transh
    unfolding k_mod.gen_nextState_def by auto
  hence "k_mod.isConc k msgh" and "k_mod.minMsgs msgh mod k = k - 1" and "x shh = 0 \<or> x shh = Suc (k_mod.minMsgs msgh)" using transh
  ...
  proof (cases "f = 2")
    case True
    have "j-1 \<ge> c" using A6[of HO xi k rho "Suc t" "j-1" q sq xi ss msgq] assms True
      sq `active_path rho HO xi q (Suc t) j` `j \<ge> k` unfolding c_def f_def msgq_def by fastforce
    hence "?P (j-Suc c) h sh shh" using sh_pre True shh `level shh = f-1` `j-Suc c mod k \<noteq> 0`
    ...
    hence "\<exists>qj sj sjj. ?P (j-Suc c) qj sj sjj" by auto
    thus "False" using Least_le[of "\<lambda>jj. \<exists>qj sj sjj. ?P jj qj sj sjj" "j-Suc c"] by (simp add: j_def)
  next
    case False
    hence "t+j-Suc c \<ge> k" using sh_pre A2[of HO xi k rho "t+j-Suc c" h sh] run loop `2 < k` star
      by (metis One_nat_def Suc_1 Suc_diff_1 Suc_diff_Suc Suc_lessD `1 < f` less_numeral_extra(4) not_le_imp_less)
    hence "\<exists>sss. rho (t+j-c-k) xi = Active sss \<and> (k_mod.isReady msgh \<longrightarrow> ready sss) \<and> x sss mod k = 0"
    ...
    hence "rho (t+j-Suc c-k) xi \<noteq> Asleep" using starting[of "t+j-c-k" rho xi k HO sss] `t+j-Suc c \<ge> k`
    ...
    hence transx:"k_mod.gen_nextState k xi ssss (HOrcvdMsgs (k_mod.gen_HOMachine k) xi (HO (t+j-c-k) xi) (rho (t+j-Suc c-k))) sss"
      ...
      by (metis Suc_diff_Suc Suc_diff_le Suc_le_lessD `Suc c \<le> t + j` `k \<le> t + j - Suc c`)
    hence "level sss > 0" using `x sss mod k = 0` `ready sss` unfolding k_mod.gen_nextState_def by auto
    hence "t+j-c-k \<ge> t" using `level s = 0` level_growing[of HO k rho "t+j-c-k" xi sss _s] s sss
    ...
    hence "?P (j-Suc c) h sh shh" using sh_pre `1 < f` shh `level shh = f-1` `j-Suc c mod k \<noteq> 0`
    ...
    hence "\<exists>qj sj sjj. ?P (j-Suc c) qj sj sjj" by auto
    thus "False" using Least_le[of "\<lambda>jj. \<exists>qj sj sjj. ?P jj qj sj sjj" "j-Suc c"] unfolding j_def by auto
  qed
qed
...
proof -
  define tt where "tt = (SOME t. \<exists>s ss. rho t xi = Active s \<and> rho (Suc t) xi = Active ss \<and> level s = 0 \<and> level ss = 1)"
    (is "tt = (SOME t. \<exists>s ss. ?P t s ss)")
  have "\<forall>p r sp ssp. rho r p = Active sp \<longrightarrow> level sp < 2 \<longrightarrow> rho (Suc r) p = Active ssp \<longrightarrow> level ssp = 2 \<longrightarrow> r mod k = tt mod k"
  proof (rule+)
    fix p r sp ssp
    ...
    moreover have "r \<ge> k" using A2[of HO xi k rho r p sp] lev2 leI assms sp unfolding k_mod.ready_level2_def by auto
    ...
    hence "rho (r-k) xi \<noteq> Asleep" using starting[of "Suc r-k" rho xi] assms `r \<ge> k` unfolding k_mod.gen_initState_def by fastforce
    ...
    hence trans:"k_mod.gen_nextState k xi ss msgs sss" using transition
      `r \<ge> k` assms sss Suc_diff_le unfolding msgs_def by fastforce
    hence "level sss > 0" using `ready sss` `x sss mod k = 0` unfolding k_mod.gen_nextState_def by auto
    thus "r mod k = tt mod k"
    proof (cases "\<exists>t s ss. rho t xi = Active s \<and> rho (Suc t) xi = Active ss \<and> level s = 0 \<and> level ss = 1")
      case True
      obtain t sxt ssxt where "?P t sxt ssxt" using True by auto
      from this obtain sx ssx where "?P tt sx ssx" using someI_ex[of "\<lambda>t. \<exists>s ss. ?P t s ss"] unfolding tt_def by auto
      hence "Suc r-k \<ge> tt" using level_growing[of HO k rho "Suc r-k" xi sss "tt-(Suc r-k)" sx] `level sss > 0`
      ...
      thus "r mod k = tt mod k" using `Suc r-k \<ge> tt` mod_add_left_eq[of "r-tt" k tt] 
        by (metis Suc_1 Suc_diff_le `k \<le> r` add.commute add_cancel_right_left add_diff_inverse_nat assms(4)
        diff_less dual_order.strict_trans dual_order.strict_trans1 less_trans_Suc not_le zero_less_Suc)
    next
      case False
      define jj where "jj = (LEAST j. \<exists>s. rho j xi = Active s \<and> level s > 0)" (is "_ = (LEAST j. \<exists>s. ?Q j s)")
      have "?Q (Suc r-k) sss" using sss `level sss > 0` by auto
      from this obtain s where s:"rho jj xi = Active s" and "level s > 0" using LeastI_ex[of "\<lambda>j. \<exists>s. ?Q j s"] unfolding jj_def by auto
      hence "rho (jj-1) xi \<noteq> Asleep" using starting[of jj rho xi k HO s] assms unfolding k_mod.gen_initState_def by auto
      ...
      hence transj:"k_mod.gen_nextState k xi sx (HOrcvdMsgs (k_mod.gen_HOMachine k) xi (HO jj xi) (rho (jj-1))) s"
        using transition s sx assms `jj > 0` by fastforce
      have "level sx = 0" using Least_le[of "\<lambda>j. \<exists>s. ?Q j s" "jj-1"] `jj > 0` sx
