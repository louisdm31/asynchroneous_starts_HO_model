...
\end{thm}
\begin{proof}
	\noindent $Roots(\mathds{G})$ is a complete with delay $D$. It must stabilize, according to the previous section.
	...
	\noindent \textbf{Claim 2:} At the latest in round $kD+D$ and beyond, we have $\forall i \in N_\mathds{G}, C_i^t \equiv C_{Roots(\mathds{G})}^t [P] \vee PM_i^t \geq PM+2D$

	This results from lemma \ref{lem:croissant}.
	...
	This results from the fact that as long as no node from $Roots(\mathds{G})$ rewinds to 0,
	...
	\noindent \textbf{Claim 6:} For any $D \geq l \geq PM+2D$, the global state verifies $\forall i \in N_\mathds{G}, C_i^{t+l} \not\equiv C_{Roots(\mathds{G})}^{t+l} \Rightarrow C_i^{t+l} \geq l-D$.

	This claim can be proved by induction over $l-D$, using claim 2 and 5 to rule out a possible rewind to 0.
