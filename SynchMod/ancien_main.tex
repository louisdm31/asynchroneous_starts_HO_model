...
\BLANK \INDENT{\textbf{Initialization:}}
	...
	\STATE $fired_u \leftarrow false$
	\STATE $delayed_u \leftarrow false$

\ENDINDENT \BLANK

\INDENT{\textbf{In each round $t$:}}
	\STATE send $\langle \overline{c}_u, concordant_u \rangle$ to all 
	\STATE receive incoming messages
	\STATE $\overline{c}_u \leftarrow$ minimum received value, incremented by 1
	\STATE $concordant_u \leftarrow$ all received values are equal to $\langle v, true \rangle \wedge$ (hence no null, no $\bot$)
	\IF{$concordant_u \wedge \overline{c}_u = k/2 \wedge \neg delayed_u$}
		\STATE $\overline{c}_u \leftarrow 1$
		\STATE $delayed_u \leftarrow true$
	\ELSE \IF{$concordant_u \wedge \overline{c}_u = 1 \wedge delayed_u$}
		\STATE $fired_u \leftarrow true$
	\ENDIF
	\ENDIF
