...
\begin{proof}
	...
	At round $t+j+1$,  the node $w(t+j+1)$ receives a message of the form $\langle x, sync_{w(t+j)}(t+j), *, * \rangle$ % PAS DE x
	from the active node $w(t+j)$ .
	From line \ref{line:conc-gossip}, we get that  $sync_{w(t+j)}(t+j)$ is true. 
	Moreover, the received messages are congruent. % A CHANGER
