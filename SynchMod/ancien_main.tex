...
At round $t+j+1$,  the node $w(t+j+1)$ receives a message of the form $\langle x, concordant_{w(t+j)}(t+j), *, * \rangle$ % PAS DE x
	from the active node $w(t+j)$ .
From line \ref{line:conc-gossip}, we get that  $concordant_{w(t+j)}(t+j)$ is true. 
Moreover, the received messages are concordant. % A CHANGER
...
%Using a backward induction along this path, we show  that any node $w(t+i)$ with $0 \leq i < k$  is active at round~$t +i $ 
%	and verifies
