...
Secondly, \textbf{a node may level-up only if it is in sync with $\gamma$.}
...
By hypothesis, $\gamma$ belongs to this set.
We want to guarantee that the value of $\gamma$ in round $t-k$ is congruent to 0.
However, since the nodes cannot know the identity of $\gamma$ from the other nodes, they conservatively level-up only if every node
...
Ultimately, every node in $In_u(t-k:t)$ (including $\gamma$) is congruent with 0 in round $t-k$.
...
\begin{algorithm}[htb]
	...
	\Indp
		...
		\If{$c_u \equiv 0 [k] \wedge (conc_u \textcolor{teal}{\vee force_u = 2)}$}{ \label{line:cond-levelup} 
			...
			\ElseIf{$\textcolor{teal}{force_u = 2 \vee}~ready_u$}{ \label{line:cond-firing}
