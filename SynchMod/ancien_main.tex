...
\INDENT{\textbf{In each round $t$:}}
	...
	\ENDIF
	\IF{$\neg tried_u $ and two messages (other than null and $\bot$) have conflicting values and no received message is $\overline{k}$ }
		\STATE $\overline{c}_u \leftarrow \overline{k} $  \label{line:try}
		\STATE $tried_u \leftarrow true$   \label{line:try+1}
	\ELSE
		\STATE $\overline{c}_u \leftarrow min \{\overline{v}+1, \overline{v} \in V\}$ where $V$ is the set of non null and non $\bot$ received values \label{line:min}
	\ENDIF
\ENDINDENT 
...
Hence, any active node $u$ at round~$t+1$ receives $\overline{k}$ in this round,
	and so updates its clock $\overline{c}_u$ according to line~\ref{line:min}.
It holds that $\overline{c}_u (t+1) =\overline{1}$.
\end{proof}
...
\begin{enumerate}
	...
	\item Induction step:  assume that the round $t+h$ is $\overline{i+h}$-monovalent,
			...
			Therefore, the node~$u$ can receive only this value (in addition of null and $\bot$), and thus updates
				$\overline{c}_u$ according to line~\ref{line:min}. 
...
\begin{lemma}\label{lem:mono_bi}
	Any round $t > \max (s(\cent), \try^{\max}) + 1$ verifying $\overline{c}_\cent(t-1) < \overline{k}$ is 
	$S$-valent with  $S \subseteq \{1, \dots, \overline{c}_\cent(t-1)+\overline{1} \, \}$.
\end{lemma}
\begin{proof}
