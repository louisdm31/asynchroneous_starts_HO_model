...
\INDENT{\textbf{Round $r$:}}
	...
	\ELSIF{$force_u \wedge$ no $k$ has been received}
		\STATE $force_u \leftarrow false$ ~~~~\COMMENT{try a "forced synchronization"}
		\STATE $\overline{c}_u \leftarrow k$
	\ELSE
		\STATE $\overline{c}_u \leftarrow 1$ ~~~~\COMMENT{in particular, if at least one $k$ has been received, adopt 1}
	\ENDIF
\ENDINDENT 
