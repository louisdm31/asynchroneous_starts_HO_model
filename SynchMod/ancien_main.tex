...
There exists a path from $u$ to $v$ in the interval $[t,t']$ if and only if $v \in In_u(t:t')$.
...
In opposition, if some node $j \in In_u(t:t+k)$ is not congruent with 0 in round $t$,
...
\begin{algorithm}[htb]
	...
	\Indp
		...
		$concordant_u \leftarrow \forall v \looparrowright u, concordant_v \wedge c_v \equiv c_u [k]$ and no null received \; \label{line:conc-gossip}
...
of node $u$ in line \ref{line:conc-levelup} during round $t$, if $u$ is active in round $t$.
...
\begin{lem} \label{lem:conc-safety}
	Let $i$ be an integer verifying $i < k$, $t$ a round. Let $u$ and $v$ be two nodes such that there exists a path from $u$ to $v$ in the interval $[t+i,t+k]$.
	If $c^{prelim}_v(t+k) \equiv 0 [k] \wedge concordant^{prelim}_v(t+k)$, then $c_u(t+i) \equiv i [k]$.
\end{lem}
\begin{proof}
	...
	We prove the base case for $w(t+k-1)$. We use the hypothesis of the lemma and the line \ref{line:conc-gossip}.
	For the induction case, we assume that $i \leq j < k$, and $c_{w(t+j+1)}(t+j+1) \equiv j+1 [k]$ and $concordant_{w(t+j+1)}(t+j+1)$.
	...
	Finally $c_{w(t+j)}(t+j) \equiv j [k]$.
\end{proof}

\noindent Using the exact same reasoning, we can prove the following lemma.
\begin{lem} \label{lem:conc-safety-bis}
	If $concordant_u \wedge ready_u \wedge c_u \equiv 0 [k]$ is true in line \ref{line:cond-levelup} in round $t+k$, then $level_\cent(t) \geq 1$.
\end{lem}
\begin{proof}
...
\begin{proof}
	...
	For the base case, from $c_u(t) = 0$, we get that $u$ leveled up in round $t$ (otherwise $c_u(t)$ would be greater than 0, see line \ref{line:min-z-end}).
...
\begin{lem} \label{lem:ready-safety}
	...
	Moreover $c^{prelim}_u \leq i$ where $c_u^{prelim}$ is the value of $c_u$ after line \ref{line:min-z-end}.
\end{lem}
\begin{proof}
	...
	We consider a path $(\cent = w(t-i), w(t-i+1) = \cent, \dots, w(t-k) = \cent, \dots, w(t) = u)$ from $\cent$ to $u$ in the interval $[t-i, t]$. 
	Using a proof similar to the proof of lemma \ref{lem:conc-safety-bis}, we can show that for any $j \leq k$,
	the node $w(t-j)$ is active, is at least in level 1, and verifies $ready_{w(t-j)}(t-j)$ and $concordant_{w(t-j)}(t-j)$.

	By induction on this path, we can show that any $w(t-j)$ on this path verifies $force_{w(t+j)}(t+j) \geq 1$ and $c_{w(t+j)}(t+j) \leq j$.
...
\begin{proof}
	...
	According to lemma \ref{lem:ready-safety}, $i$ and $j$ must be non-negative.
