...
\INDENT{\textbf{In each round $t$:}}
	\STATE send $\langle \overline{c}_u, concordant_u, delayed_u \rangle$ to all 
	\STATE receive incoming messages
	\IF{at least a message raised the $delayed$ flag}
		\STATE $delayed_u \leftarrow true$
		\STATE $\overline{c}_u \leftarrow min \{v+1, \langle v, *, true \rangle received \}$
	\ELSE
	...
	\IF{$concordant_u \wedge \overline{c}_u = k/2 \wedge \neg delayed_u$}
		\STATE $\overline{c}_u \leftarrow k/4$
		\STATE $delayed_u \leftarrow true$
	\ENDIF
	\IF{$concordant_u \wedge \overline{c}_u = 1 \wedge delayed_u$}
		\STATE $fired_u \leftarrow true$
	\ENDIF
