...
\begin{algorithm}[htb]
	...
	\Indp
		...
		\If{$c_u \equiv 0 [k]$}{ \label{line:cond-levelup} 
			\If{$level_u = 0 \wedge concordant_u$}{ \label{line:level1up}
				$level_u \leftarrow 1$ \;
				$force_u \leftarrow max(1, force_u)$ \; \label{line:force2}
				$c_u \leftarrow 0$ \; \label{line:force3}
			}
			\ElseIf{$level_u = 1 \wedge ready_u \wedge concordant_u$}{ \label{line:cond-firing}
				$force_u \leftarrow 2$ \;
...
\begin{proof}
	...
	Using $\mathcal{P}_{k-centered}$, we obtain $\cent \in In_u(t-i:t)$.
...
\begin{proof}
	...
	Then, using $\mathcal{P}_{k-centered}$, we obtain $\cent \in In_u(t:t+i)$.
