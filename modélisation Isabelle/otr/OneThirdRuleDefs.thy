...
definition HOV :: "(Proc \<Rightarrow> 'val message) \<Rightarrow> 'val \<Rightarrow> Proc set" where
  "HOV msgs v \<equiv> { q . msgs q = Content v }"
...
definition MFR :: "(Proc \<Rightarrow> 'val message) \<Rightarrow> 'val \<Rightarrow> bool" where
...
lemma MFR_in_msgs:
  assumes HO:"HOs m p \<noteq> {}"
      and v: "MFR (HOrcvdMsgs OTR_M p (HOs m p) (rho m)) v"
             (is "MFR ?msgs v")
