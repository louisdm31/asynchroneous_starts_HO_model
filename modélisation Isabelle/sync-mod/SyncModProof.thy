...
shows "\<exists>s ss. rho r q = Active s \<and> rho (Suc r) q = Active ss \<and> x ss = v"
proof (cases "rho (Suc r) q")
    ...
    thus "\<exists>s ss. rho r q = Active s \<and> rho (Suc r) q = Active ss \<and> x ss = v" using \<open>?msgs q = Content (Val v)\<close> by auto
next
    case (Active sq)
    have "sendMsg ?A q p sq = Val (x sq)"
        using HOrcvdMsgs_def[of ?A p "HO (Suc (Suc r)) p" "rho (Suc r)"] \<open>?msgs q = Content (Val v)\<close>
    ...
    hence "rho r q \<noteq> Aslept"
        using stating[of "Suc r" rho q k HO sq] \<open>rho (Suc r) q = Active sq\<close>
    ...
    ultimately have "rho r q = Active s \<and> rho (Suc r) q = Active sq \<and> x sq = v"
        using \<open>rho (Suc r) q = Active sq\<close> \<open>?msgs q = Content (Val v)\<close> k_mod.SyncMod_sendMsg_def[of k q p sq] by auto
    thus "\<exists>s ss. rho r q = Active s \<and> rho (Suc r) q = Active ss \<and> x ss = v"  by auto
qed
...
proof -
    ...
    proof (cases "k_mod.ready_force k ?msgs s")
        case True
        thus "x ss < k" using nxt k_mod.SyncMod_nextState_def \<open>k > 2\<close> by auto
    next
        case False
        thus "x ss < k"
        proof (cases "x ss = 0")
            case True
            thus "x ss < k" using \<open>k > 2\<close> by auto
        next
            case False
            hence "(x ss) mod k = x ss" using nxt k_mod.SyncMod_nextState_def by auto
            thus "x ss < k" using \<open>k > 2\<close> by (metis less_Suc_eq_0_disj less_imp_Suc_add mod_less_divisor)
        qed
    qed
...
proof (induction i)
    from assms(6) show \<open>monovalent rho ((v+0) mod k) (r+0)\<close> by auto
next
    case (Suc i)
    ...
    hence "\<forall>p. k_mod.SyncMod_sendMsg k xi p sxx = Val ((v+i) mod k)" using k_mod.SyncMod_sendMsg_def by 
    ...
    hence "\<forall>p. sendMsg ?A xi p sxx = Val ((v+i) mod k)"
        using HOMachine_to_Algorithm_def k_mod.SyncMod_HOMachine_def by (simp add: k_mod.SyncMod_HOMachine_def)
    hence "\<forall>p. ?msgs p xi = Content (Val ((v+i) mod k))" (is "\<forall>p. ?msgs p xi = _")
        using assms(1) by (simp add: HOrcvdMsgs_def k_mod.xi_nek_def \<open>rho (Suc (r + i)) xi = Active sxx\<close>)

    hence concord:"\<forall>p. k_mod.concordant (?msgs p) ((v+i) mod k)"
    ...
    proof (rule+)
        ...
        proof (cases "k_mod.ready_force k (?msgs p) ss")
            ...
            thus "x st = (v + Suc i) mod k" using \<open>v1 \<noteq> v2\<close> by auto
        next
            case False
            thus "x st = (v + Suc i) mod k"
            proof (cases "x st = 0")
                case True
                hence "\<forall>vv. k_mod.concordant (?msgs p) vv \<longrightarrow> vv =  k - 1"
                    using \<open>k_mod.SyncMod_nextState k p ss (?msgs p) st\<close> k_mod.SyncMod_nextState_def
                    \<open>\<not> k_mod.ready_force k (?msgs p) ss\<close> by auto
                hence "(v+i) mod k = k - 1" using concord k_mod.concordant_def by auto
                thus "x st = (v+Suc i) mod k" using \<open>x st = 0\<close>
            ...
            next
                ...
                thus "x st = (v+Suc i) mod k" using \<open>(x st) mod k = x st\<close>
...
proof -
    ...
    then obtain n :: nat where "!p. rho n p \<noteq> Aslept" by auto
    ...
    have "\<forall>p r. rho r p \<noteq> Aslept --> (\<exists>rf sf. rho rf p = Active sf & fire sf)"
    proof (rule allI impI)+
        fix p
        fix r :: nat
        assume "rho r p \<noteq> Aslept"
        moreover have "1 \<le> (n+1)*k" using \<open>k > 2\<close> by auto
        hence "r+((n+1)*k-1) = r+(n+1)*k-1" (is "_ = ?rd") using  ordered_cancel_comm_monoid_diff_class.add_diff_assoc[of 1 "(n+1)*k" r] by auto
        ultimately have "rho (Suc ?rd) p \<noteq> Aslept" using nonAsleepAgain[of rho r p _ _ _ "(n+1)*k"] run HORun_def
            by (smt \<open>1 \<le> (n + 1) * k\<close> add.commute add_Suc_right ordered_cancel_comm_monoid_diff_class.add_diff_inverse plus_1_eq_Suc)
        then obtain ss where "rho (Suc ?rd) p = Active ss" by (cases "rho (Suc ?rd) p") auto
        moreover from this have "rho (Suc (Suc ?rd)) p \<noteq> Aslept"
        ...
        have "\<forall>q. ?msgs p = Void \<or> ?msgs p = Content (Val (k-1))"
        proof
            fix q
            have "?rd-n = r+n*k+1*k-1-n" using \<open>k > 2\<close> by simp
            hence positiv:"?rd-n > 0" using \<open>k > 2\<close>
            ...
            hence "rho ?rd q ~= Aslept"
                using nonAsleepAgain[of rho n q ?A HO "\<lambda>r q. undefined" "?rd-n"]
                run HORun_def[of ?A rho HO] \<open>\<forall>p. rho n p ~= Aslept\<close> \<open>k > 2\<close> by auto
            then obtain sq where "rho ?rd q = Active sq" by (cases "rho ?rd q") auto
            moreover have "rho (Suc ?rd) q ~= Aslept"
                using run HORun_def[of ?A rho HO] \<open>k > 2\<close>
                by (metis \<open>rho ?rd q ~= Aslept\<close> nonAsleepAgain plus_1_eq_Suc)
            then obtain sqq where "rho (Suc ?rd) q = Active sqq" by (cases "rho (Suc ?rd) q") auto
            moreover have "r+(n+1)*k > 0" using \<open>k > 2\<close> by auto
            ...
            moreover have "monovalent rho ((k - 1) :: nat) (?rd :: nat)" using monoval by auto
                ...
                hence "k_mod.SyncMod_sendMsg k q p sqq = Val (k-1)"
                    using k_mod.SyncMod_sendMsg_def[of k q p sqq] \<open>k > 2\<close> by auto 
                hence "sendMsg ?A q p sqq = Val (k-1)"
                    using k_mod.SyncMod_HOMachine_def HOMachine_to_Algorithm_def
                    by (simp add: `\<And>k. k_mod.SyncMod_HOMachine k \<equiv> (| CinitState = \<lambda>p st crd. k_mod.SyncMod_initState k p st,
                    sendMsg = k_mod.SyncMod_sendMsg k, CnextState = \<lambda>p st msgs crd. k_mod.SyncMod_nextState k p st msgs,
        ...
        show "\<exists>rf sf. rho rf p = Active sf \<and> fire sf"